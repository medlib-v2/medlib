<div class="right-side-section">
		...
		<div class="panel-body nopadding">
			<div class="mini-profile socialite">
				<div class="background">
					<div class="widget-bg">
						<img src=" @if(Auth::user()->cover) {{ url('user/cover/'.Auth::user()->cover) }} @else {{ url('user/cover/default-cover-user.png') }} @endif" alt="{{ Auth::user()->getName() }}" title="{{ Auth::user()->getName() }}">
					</div>
					<div class="avatar-img">
						<img src="{{ Auth::user()->getAvatar() }}" alt="{{ Auth::user()->getName() }}" title="{{ Auth::user()->getName() }}">
					</div>
				</div>
				<div class="avatar-profile">
					<div class="avatar-details">
						<h2 class="avatar-name">
							<a href="{{ url(Auth::user()->getUsername()) }}">{{ Auth::user()->getName() }}</a>
						</h2>
						<h4 class="avatar-mail">
							<a href="{{ url(Auth::user()->getUsername()) }}">{{ '@'.Auth::user()->getUsername() }}</a>
						</h4>
					</div>
				</div>
				<ul class="activity-list list-inline">
					<li>
						<a href="{{ url(Auth::user()->username.'/posts') }}">
							<div class="activity-name">{{ trans('common.posts') }}</div>
							<div class="activity-count">{{ Auth::user()->feeds->count() }}</div>
						</a>
					</li>
					<li>
						<a href="{{ url(Auth::user()->getUsername() .'/followers') }}">
							<div class="activity-name">{{ trans('common.followers') }}</div>
							<div class="activity-count">{{ Auth::user()->followers->count() }}</div>
						</a>
					</li>
					<li>
						<a href="{{ url(Auth::user()->getUsername() .'/following') }}">
							<div class="activity-name">{{ trans('common.following') }}</div>
							<div class="activity-count">{{ Auth::user()->following->count() }}</div>
						</a>
					</li>
				</ul>
			</div><!-- /mini-profile -->
	...
	<div class="panel panel-default">
			...
			<div class="user-follow socialite">
				...
				<div class="media">
					<div class="media-left badge-verification">
						<a href="{{ url($suggested_user->getUsername()) }}">
							<img src="{{ $suggested_user->getAvatar() }}" class="img-icon" alt="{{ $suggested_user->getName() }}" title="{{ $suggested_user->getName() }}">
					...
					<div class="media-body socialte-timeline follow-links">
						<h4 class="media-heading"><a href="{{ url($suggested_user->getUsername()) }}">{{ $suggested_user->getName() }} </a>
							<span class="text-muted">{{ '@'.$suggested_user->getUsername() }}</span>
						</h4>
							<div class="btn-follow">
								<a href="#" class="btn btn-default follow-user follow" data-timeline-id="{{ $suggested_user->timeline_id }}"> <i class="fa fa-heart"></i> {{ trans('common.follow') }}</a>
							</div>
							<div class="btn-follow hidden">
								<a href="#" class="btn btn-success follow-user unfollow" data-timeline-id="{{ $suggested_user->timeline_id }}"><i class="fa fa-check"></i> {{ trans('common.following') }}</a>
							</div>
						</div>
					</div>
					@endforeach
		...
		<div class="panel panel-default">
			...
			@if(settings('home_ad') != NULL)
			<div id="link_other" class="post-filters">
				{!! htmlspecialchars_decode(settings('home_ad')) !!}
			</div>
			@endif
		</div>
