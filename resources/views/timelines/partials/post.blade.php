  ...
  <div class="panel-heading no-bg">
    <div class="post-author">
        ...
        <ul class="list-inline no-margin">
          <li class="dropdown"><a href="#" class="dropdown-togle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><i class="fa fa-angle-down"></i></a>
            <ul class="dropdown-menu">
              @if($post->follows->contains(Auth::user()->id))
              <li class="main-link">
                <a href="#" data-post-id="{{ $post->id }}" class="notify-user unnotify">
                  <i class="fa  fa-bell-slash" aria-hidden="true"></i>{{ trans('common.stop_follows') }}
                  <span class="small-text">{{ trans('messages.stop_follows_text') }}</span>
                </a>
              </li>
              <li class="main-link hidden">
                <a href="#" data-post-id="{{ $post->id }}" class="notify-user notify">
                  <i class="fa fa-bell" aria-hidden="true"></i>{{ trans('common.get_follows') }}
                  <span class="small-text">{{ trans('messages.get_follows_text') }}</span>
                </a>
              </li>
              @else
              <li class="main-link hidden">
      ...
      <div class="user-avatar">
        <a href="{{ url($post->user->getUsername()) }}"><img src="{{ $post->user->getAvatar() }}" alt="{{ $post->user->getName() }}" title="{{ $post->user->getName() }}"></a>
      </div>
      <div class="user-post-details">
        <ul class="list-unstyled no-margin">
          <li>
            <a href="{{ url($post->user->getUsername()) }}" title="{{ '@'.$post->user->getUsername() }}" data-toggle="tooltip" data-placement="top" class="user-name user">
              {{ $post->user->getName() }}
            </a>
                ...
                @if($key==1)
                  {{ trans('common.and') }}
                    @if(count($post_tags)==1)
                      <a href="{{ url($user->getUsername() ) }}"> {{ $user->getName() }}</a>
                    @else
                  ...
                  @break
                @endif
                <a href="{{ url($user->getUsername()) }}" class="user"> {{ array_shift($post_tags) }} </a>
              @endforeach
    ...
    <div class="panel-body">
      ...
      <ul class="actions-count list-inline">
        ...
        <?php
        $liked_ids = $post->likes->pluck('id')->toArray();
        $liked_names = $post->likes->pluck('name')->toArray();
        ?>
        <li>
          <a href="#" class="show-users-modal" data-html="true" data-heading="{{ trans('common.likes') }}"  data-users="{{ implode(',', $liked_ids) }}" data-original-title="{{ implode('<br />', $liked_names) }}"><span class="count-circle"><i class="fa fa-thumbs-up"></i></span> {{ $post->likes->count() }} {{ trans('common.likes') }}</a>
        </li>
        @endif
    ...
    <div class="panel-footer socialite">
      <ul class="list-inline footer-list">
        ...
        @if(Auth::user()->id != $post->user_id)
          @if(!$post->shared->contains(Auth::user()->id))
          ...
          <div class="comments post-comments-list"> <!-- comments/main-comment  -->
            @if($post->comments->count() > 0)
              @foreach($post->comments as $comment)
                @include('timelines.partials.comment', compact('comment','post'))
              @endforeach
            @endif
