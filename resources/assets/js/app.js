...
(function($){
     ...
     * Events:
     *   medlib-app:loaded - fires after pjax request is made and ALL scripts are trully loaded
     *   medlib-app:content-resize - fires when .content changes its size (e.g. switching between static & collapsing
     *     navigation states)
     * @constructor
     */
    var MedlibAppView = function(){
        ...
        this.settings = window.MedlibSettings;
        ...
        ('ontouchstart' in window) && this.$content.swipe({
            ...
            threshold: Medlib.isScreen('xs') ? 100 : 200
        });
        ...
        if (this.pjaxEnabled){
            ...
            $(document).on('medlib-app:loaded', $.proxy(this._loadingFinished, this));
            $(document).on('medlib-app:loaded', $.proxy(this._collapseNavIfSmallScreen, this));
            $(document).on('medlib-app:loaded', $.proxy(this.hideLoader, this));
     ...
     * @private
     */
    MedlibAppView.prototype._initOnResizeCallbacks = function(){
        var resizeTimeout,
            view = this;

        $(window).on('resize medlib-app:content-resize', function() {
     ...
     * @private
     */
    MedlibAppView.prototype._initOnScreenSizeCallbacks = function(){
        var resizeTimeout,
            view = this,
            prevSize = Medlib.getScreenSize();

        $(window).resize(function() {
            clearTimeout(resizeTimeout);
            resizeTimeout = setTimeout(function(){
                var size = Medlib.getScreenSize();
    ...
    MedlibAppView.prototype._resetResizeCallbacks = function(){
     ...
     */
    MedlibAppView.prototype.checkNavigationState = function(){
        if (this.isNavigationStatic()){
            this.staticNavigationState();
            if (Medlib.isScreen('sm') || Medlib.isScreen('xs')){
                this.collapseNavigation();
            }
        } else {
            if (Medlib.isScreen('md') || Medlib.isScreen('lg')){
                var view = this;
                setTimeout(function(){
     ...
     */
    MedlibAppView.prototype.toggleNavigationCollapseState = function(){
    ...
    MedlibAppView.prototype.collapseNavigation = function(){
        //this method only makes sense for non-static navigation state
        if (this.isNavigationStatic() && (Medlib.isScreen('md') || Medlib.isScreen('lg'))) return;
    ...
    MedlibAppView.prototype.expandNavigation = function(){
        //this method only makes sense for non-static navigation state
        if (this.isNavigationStatic() && (Medlib.isScreen('md') || Medlib.isScreen('lg'))) return;
    ...
    MedlibAppView.prototype._sidebarMouseEnter = function(){
        if (Medlib.isScreen('md') || Medlib.isScreen('lg')){
            this.expandNavigation();
        }
    };

    MedlibAppView.prototype._sidebarMouseLeave = function(){
        if (Medlib.isScreen('md') || Medlib.isScreen('lg')){
            this.collapseNavigation();
        }
    };

    MedlibAppView.prototype._collapseNavIfSmallScreen = function(){
        if (Medlib.isScreen('xs') || Medlib.isScreen('sm')){
            this.collapseNavigation();
        }
    };
     ...
     */
    MedlibAppView.prototype.toggleNavigationState = function(){
        if (this.isNavigationStatic()){
            this.collapsingNavigationState();
        } else {
            this.staticNavigationState();
        }
        $(window).trigger('medlib-app:content-resize');
    };
     ...
     */
    MedlibAppView.prototype.staticNavigationState = function(){
     ...
     * Static navigation state - navigation stays always open.
     */
    MedlibAppView.prototype.collapsingNavigationState = function(){
        this.settings.set('nav-static', false).save();
        $('body').removeClass('nav-static');
        this.collapseNavigation();
    };

    MedlibAppView.prototype.isNavigationStatic = function(){
     ...
     * @private
     */
    MedlibAppView.prototype._changeActiveNavigationItem = function(event, xhr, options){
     ...
     * @private
     */
    MedlibAppView.prototype._contentSwipeLeft = function(){
        //this method only makes sense for small screens + ipad
        if (Medlib.isScreen('lg')) return;
     ...
     * @private
     */
    MedlibAppView.prototype._contentSwipeRight = function(){
        //this method only makes sense for small screens + ipad
        if (Medlib.isScreen('lg')) return;

        // fixme. this check is bad. I know. breaks loose coupling principle
        // MedlibApp should not know about some "strange" sidebar chat.
    ...
    MedlibAppView.prototype.showLoader = function(){
        var view = this;
    ...
    MedlibAppView.prototype.hideLoader = function(){
     ...
     */
    MedlibAppView.prototype.onResize = function(fn, /**Boolean=*/ allPages){
     ...
     * @param fn A function to execute
     */
    MedlibAppView.prototype.onPageLoad = function(fn){
     ...
     */
    MedlibAppView.prototype.onScreenSize = function(size, fn, /**Boolean=*/ onEnter){
     ...
     */
    MedlibAppView.prototype.pageLoaded = function(){
     ...
     * @private
     */
    MedlibAppView.prototype._addPageCallback = function(callbacks, fn){
     ...
     * @private
     */
    MedlibAppView.prototype._runPageCallbacks = function(callbacks){
        var pageName = this.extractPageName(location.href);
        if (callbacks[pageName]){
     ...
     * @private
     */
    MedlibAppView.prototype._loadScripts = function(event, data, status, xhr, options){
        ...
        var view = this;
        $previous.load(function(){
            $(document).trigger('medlib-app:loaded');
            view.log('scripts loaded.');
        })
    };

    MedlibAppView.prototype.extractPageName = function(url){
    ...
    MedlibAppView.prototype._checkLoading = function(e){
    ...
    MedlibAppView.prototype._loadingFinished = function(){
        this.loading = false;
    };

    MedlibAppView.prototype._logErrors = function(){
        var errors = JSON.parse(localStorage.getItem('lb-errors')) || {};
        errors[new Date().getTime()] = arguments;
        localStorage.setItem('medlib-errors', JSON.stringify(errors));
        //this.debug && console.log('check errors');
    };

    MedlibAppView.prototype.log = function(message){
        if (this.debug){
            console.log("Medlib Application : "
                    + message
    ...
    window.MedlibApp = new MedlibAppView();

    //MedlibApp.expandNavigation();
...
function initAppFunctions(){
    !function($){
        ...
        MedlibApp.onScreenSize('xs', moveNotificationsDropdown);
        MedlibApp.onScreenSize('xs', moveBackNotificationsDropdown, false);

        Medlib.isScreen('xs') && moveNotificationsDropdown();
        ...
        MedlibApp.onResize(initSidebarScroll, true);
        initSidebarScroll();
    ...
    !function($){
        ...
        $(document).on('click', '[data-toggle=chat-sidebar]', function(){
            ...
            $(this).find('.chat-notification-medlib').remove();
        });
        ...
        $('#content').on('swipeLeft', function(e){
            ...
            if ($('body').is('.nav-collapsed.chat-sidebar-opened')){
                $chatContainer.removeClass('chat-sidebar-opened')
                    ...
                    // used by MedlibApp to check whether it is permitted to open navigation
                    // on swipeRight
        ...
        MedlibApp.onResize(initChatSidebarScroll, true);
...
/**
 * Demo-only functions. Does not affect the core Medlib functionality.
