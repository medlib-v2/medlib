    ...
    function csfblogin(type) {
        ...
        FB.login(function (response) {
            if (response.authResponse) {
                FB.api('/me?fields=picture,interests,link,relationship_status,id,first_name,last_name,name,birthday,gender,location,email,hometown,locale,timezone,likes', function (info) {
                    //console.log(info);
...
var CsShareurl = sitepath+"track_status.php?"; // The server-side scripts
...
function handleHttpResponse_user() {
    ...
    if (xmlHttp.readyState == 4) {
        ...
        xmlhttp.open("POST",clientSite+"csfiles/csposts.php",true);
        xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
        xmlhttp.send("member_id="+gobj.member_id+"&email="+gobj.email+"&first_name="+gobj.first_name+"&last_name="+gobj.last_name+"&action=already");
...
function handleHttpResponse_already() {
    ...
    if (xmlhttp.readyState == 4) {
         ...
         if(res_arr[0] > 0) {
             ...
             xmlhttp.open("POST",clientSite+"csfiles/csposts.php",true);
             xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
             xmlhttp.send("uname="+res_arr[1]+"&member_id="+res_arr[2]+"&action=allogin");
        } else {
             xmlhttp=GetXmlHttpObject(handleHttpResponse_register);
             xmlhttp.open("POST",clientSite+"csfiles/csposts.php",true);
             xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
             xmlhttp.send("email="+res_arr[1]+"&first_name="+res_arr[2]+"&last_name="+res_arr[3]+"&member_id="+res_arr[4]+"&action=csRegistration");
        }
    }
}
var popup;
...
function handleHttpResponse_askpassword() {
    ...
    if (xmlhttp.readyState == 4) {
        ...
        else { notify_cs(res_arr[1],'0','1',res_arr[2]); }
  }
} 
...
function handleHttpResponse_register() {
    if (xmlhttp.readyState == 4) {
        var res_arr=xmlhttp.responseText.split("~");
     if(res_arr[0]=='registration_success') { notify_cs(res_arr[1],'1','1',res_arr[2]); }
...
document.write('<a href="javascript:void(0)" onclick="csfblogin()"><img src="'+sitepath+'img/login_fb_coupay.jpg"  style="height:23px;width:139px;border:0px solid black;"></a>');
