(function($){
    ...
    function initPointSparkline($el, data){
        $el.sparkline(data, {
            ...
            width: '100%',
            height: '60',
            lineColor: Medlib.colors['gray'],
            fillColor: 'transparent',
            spotRadius: 5,
            spotColor: Medlib.colors['gray'],
            valueSpots: {'0:':Medlib.colors['gray']},
            highlightSpotColor: Medlib.colors['white'],
            highlightLineColor: Medlib.colors['gray'],
            minSpotColor: Medlib.colors['gray'],
            maxSpotColor: Medlib.colors['brand-danger'],
    ...
    function initChangesChart(){
        ...
        var graph = new Rickshaw.Graph( {
            ...
            series: [{
                ...
                color: Medlib.lighten(Medlib.colors['brand-success'], .09),
                renderer: 'bar'
            }, {
                name: 'humidity',
                data: seriesData.shift().map(function(d) { return { x: d.x, y: d.y * (Math.random()*0.1 + 1.1) } }),
                renderer: 'line',
                color: Medlib.colors['white']
            }]
        } );
    ...
    function initChangesYearChart(){
        ...
        $el.sparkline(backgroundData,{
            type: 'bar',
            height: 26,
            barColor: Medlib.colors['gray-lighter'],
            barWidth: 7,
            barSpacing: 5,
        ...
        $el.sparkline(data,{
            composite: true,
            type: 'bar',
            barColor: Medlib.colors['brand-success'],
            barWidth: 7,
            barSpacing: 5
        });
    }
    ...
    function initSalesChart(){
        ...
        function _initChart(){
            $.plot($("#chart-stats-simple"), [{
                ...
                colors: [Medlib.darken(Medlib.colors['gray-lighter'], .05), Medlib.colors['brand-danger']]
            });
        }

        _initChart();

        MedlibApp.onResize(_initChart);
    }
        ...
        for (i = 0; i < 25; i++){
         ...
         function _initChart(){
             $.plot($("#chart-stats-simple-2"), [{
                 ...
                 colors: ['#777', Medlib.colors['brand-warning']]
             });
         }
    ...
    function initRealTime1(){
        ...
        var graph = new Rickshaw.Graph( {
            ...
            series: [
                {
                    color: Medlib.colors['gray-dark'],
                    data: seriesData[0],
                    name: 'Uploads'
                }, {
                    color: Medlib.colors['gray'],
                    data: seriesData[1],
                    name: 'Downloads'
                }
            ]
    ...
    function initYearsMap(){
        ...
        $map.mapael({
            map:{
                ...
                defaultArea : {
                    attrs : {
                        fill: Medlib.colors['gray-lighter']
                        , stroke : Medlib.colors['gray']
                    },
                    attrsHover : {
                        fill : Medlib.colors['gray-light'],
                        animDuration : 100
                    }
                },
                defaultPlot:{
                    size: 17,
                    attrs : {
                        fill : Medlib.colors['brand-warning'],
                        stroke : "#fff",
                        "stroke-width" : 0,
    ...
    function initWeather(){
        var icons = new Skycons({"color": Medlib.colors['white']});
        icons.set("clear-day", "clear-day");
        icons.play();

        icons = new Skycons({"color": Medlib.colors['white']});
        ...
        icons = new Skycons({"color": Medlib.lighten(Medlib.colors['brand-warning'], .1)});
        ...
        icons = new Skycons({"color": Medlib.colors['brand-success']});
        icons.set("partly-cloudy-day-1", "partly-cloudy-day");
        icons.play();

        icons = new Skycons({"color": Medlib.colors['gray']});
        icons.set("clear-day-2", "clear-day");
        icons.play();

        icons = new Skycons({"color": Medlib.colors['gray-light']});
