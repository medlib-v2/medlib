...
import { notifications as http } from '@/services'
...
export default{
    ...
    mounted() {
        http.fetch().then( ({total, notifications}) => {
            ...
            **/
            notifications.forEach( (notification) => {
                this.setNotificationUnread(notification)
            })
        })
    ...
    methods: {
        ...mapActions(['setNotificationUnread']),
        listen() {
            /**
        ...
        allowFriendRequest(username, id) {
            http.acceptRequest(username, id).then( (response) => {
                /**
                 * Set notification
                 */
        ...
        denyFriendRequest(username, id) {
            http.cancelRequest(username, id).then( (response) => {
                /**
                 * Set notification
                 */
