...
window.fbAsyncInit = function() {
    ...
    FB.Event.subscribe('edge.create', function(response) {
        FB.api('/me', function (info) {console.log(info); });
    });

    FB.Event.subscribe('edge.remove', function(href, widget) { alert('You just Disliked '+href);});
};
...
function handleHttpResponse_reg() {
    ...
    if (xmlHttp_149.readyState == 4) { alert("Referral is tracked");}
}
...
function handleHttpResponse_dilip() {
    ...
    if (xmlHttp_149.readyState == 4) { alert("Clcik is tracked");}
}
    ...
    FB.getLoginStatus(function(response) {
        if (response.status === 'connected') {
            ...
            FB.ui({
                ...
                if (response && !response.error_code) { alert(response); }
            });
        }
        FB.ui({
            method: 'share',
            ...
            if (response && !response.error_code) {
                FB.api('/me', function (info) { alert("shared by"+info.first_name+"  "+info.id); });
            }
        });
    });
}
...
function login(type) {
    ...
    FB.login(function (response) {
        if (response.authResponse) {
            if(type=='1') {
                FB.api('/v2.0/me?fields=picture,interests,link,relationship_status,id,first_name,last_name,name,birthday,gender,location,email,hometown,locale,timezone,friends,likes', function (info) {track_url_new(info);});
            } else {}
...
if(getQueryStringValue("csid")!= '') {
    ...
    if(getQueryStringValue("is_fb")) { var is_from_cookie = '1'; }
    if(getQueryStringValue("is_li")) { var is_from_cookie = '2'; }
    if(getQueryStringValue("is_tw")) { var is_from_cookie = '3'; }
...
function checkDecimalsCS(fieldValue) {
    ...
    if (isNaN(fieldValue) || fieldValue == "") { return false; }
    else {
