...
var Medlib = (function () {
    ...
    Medlib.Pushed = function() {
        ...
        $.fn.getLocation = function (url) {
            $("#geoLoader").html("<img src='"+ url +"images/ajax-loader.gif' class='padding10'/> Loading Geo Location");
        ...
        function loadData() {
            $('div.postloader').html('<img src="'+  window.location.hostname +'/images/ajax-loader.gif">');
        ...
        function handleValidateAndSubmit(event) {
            ...
            var form = $(this),
                ...
                senderProfileImage = form.find("input[name='senderProfileImage']"),
                image = form.find("input[name='image']"),
                videoUrl = form.find("input[name='videoUrl']"),
                location = form.find("input[name='location']"),
                place = form.find('#geocomplete'),
                map_canvas = form.find("#map_can");
            ...
            if(shareType.val() == 'photos') {
                ...
                if (filename == "") {
                    $.Notification.add({
                        title: 'Top Left',
                        text: "<span class='error'>Please Browse to upload a valid image file</span>"+"<h2>Note</h2>"+"<span class='error_message'>Only JPEG, JPG and PNG Images type allowed</span>",
                        className: 'error'
                    });
                    return false;
                }
                ...
                if(!((imagefile == match[0]) || (imagefile == match[1]) || (imagefile == match[2]))) {
                    $.Notification.add({
                        title: 'Top Left',
                        text: "<span class='error'>Please select a valid image file</span>"+"<h2>Note</h2>"+"<span class='error_message'>Only JPEG, JPG and PNG Images type allowed</span>",
                        className: 'error'
                    });
                    return false;
                }
            ...
            if(shareType.val() == 'video') {
                if (videoUrl.val() == '' && videoUrl.attr('name') == 'videoUrl') {
                    $.Notification.add({
                        title: 'Top Left',
                        text: "Video Url could not be empty",
                        className: 'error'
                    });
                    return false;
                }
                ...
                if(video == false) {
                    $.Notification.add({
                        title: 'Top Left',
                        text: "Not a valid <b>Youtube/Vimeo</b> video URL",
                        className: 'error'
                    });
                    return false;
                }
